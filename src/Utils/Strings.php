...
class Strings
{
    ...
    public static function truncate(string $s, int $maxLen, string $append = "\u{2026}") : string
    {
        if (self::length($s) > $maxLen) {
            ...
            if ($maxLen < 1) {
                return $append;
            } else if ($matches = self::match($s, '#^.{1,' . $maxLen . '}(?=[\s\x00-/:-@\[-`{-~])#us')) {
    ...
    /**
     * Performs a global regular expression match. Accepts flag PREG_OFFSET_CAPTURE (returned in bytes), PREG_SET_ORDER
     * is default.
     */
    ...
    public static function replace(string $subject, $pattern, $replacement = null, int $limit = -1) : string
    {
        ...
        } else if ($replacement === null && is_array($pattern)) {
