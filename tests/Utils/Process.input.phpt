...
test('string as input', function () {
	$input = 'Hello Input';
	$process = Process::runExecutable(getPhpCliBinary(), ['-r', 'echo fgets(STDIN);'], stdin: $input);
...
test('large string input', function () {
	...
	$process = Process::runExecutable(getPhpCliBinary(), ['-r', 'echo strlen(stream_get_contents(STDIN));'], stdin: $input);
	Assert::same('200000', $process->getStdOutput());
});

test('invalid input type is rejected before the process starts', function () {
	Assert::exception(
		fn() => Process::runExecutable(getPhpCliBinary(), ['-r', 'sleep(10);'], stdin: false),
...
test('write input', function () {
	$process = Process::runExecutable(getPhpCliBinary(), ['-r', 'echo fgets(STDIN);'], stdin: null);
...
test('writeStdInput() throws exception when stdin is not null', function () {
	$process = Process::runExecutable(getPhpCliBinary(), ['-r', 'echo fgets(STDIN);']);
	Assert::exception(
