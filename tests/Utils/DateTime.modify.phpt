...
test('Complex and varied format strings', function () {
	...
	$dt->modify('first day of may noon');
...
test('Invalid modifier format exceptions', function () {
	Assert::exception(
		fn() => (new DateTime('now'))->modify('+'),
		Throwable::class,
		"#Invalid modifier '\\+'#",
	);

	Assert::exception(
		fn() => (new DateTime('now'))->modify('1 month 2'),
		Throwable::class,
		"#Invalid modifier '1 month 2'#",
	);

	Assert::exception(
		fn() => (new DateTime('now'))->modify('invalid modifier string'),
		Throwable::class,
		'%a%invalid modifier string%a%',
	);
});
