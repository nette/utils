...
(function () use ($main) { // resizing Y shrink
	$image = clone $main;
...
(function () use ($main) { // resizing X Y shrink
	$image = clone $main;
...
(function () use ($main) { // resizing X Y
	$image = clone $main;
...
(function () use ($main) { // resizing X Y stretch
	$image = clone $main;
...
(function () use ($main) { // resizing X Y shrink stretch
	$image = clone $main;
...
(function () use ($main) { // flipping Y shrink
	$image = clone $main;
	$image->resize(NULL, -150, Image::SHRINK_ONLY);
	Assert::same(176, $image->width);
	Assert::same(104, $image->height);
...
(function () use ($main) { // flipping X Y shrink
	$image = clone $main;
	$image->resize(-300, -150, Image::SHRINK_ONLY);
	Assert::same(176, $image->width);
	Assert::same(104, $image->height);
...
(function () use ($main) { // exact resize
	$image = clone $main;
...
(function () use ($main) { // alpha resize
...
(function () use ($main) { // palette alpha resize
