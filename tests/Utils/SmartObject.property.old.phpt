...
Assert::error(function () {
	$obj = new TestClass;
	$obj->foo = 'hello';
}, E_USER_DEPRECATED, 'Use setFoo() or add annotation @property for TestClass::$foo in ' . __FILE__ . ':' . (__LINE__ - 1));

Assert::error(function () {
	$obj = new TestClass;
	$val = $obj->foo;
}, E_USER_DEPRECATED, 'Use getFoo() or add annotation @property for TestClass::$foo in ' . __FILE__ . ':' . (__LINE__ - 1));
