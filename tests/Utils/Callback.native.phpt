...
Assert::same( 'trim', getName(Callback::create('trim')->toReflection()) );

Assert::same( array('Test', 'add'), Callback::create('Test', 'add')->getNative() );
Assert::same( 'Test::add', (string) Callback::create('Test', 'add') );
Assert::same( 'Test::add', getName(Callback::create('Test', 'add')->toReflection()) );
...
Assert::same( 'Test::__invoke', getName(Callback::create($test)->toReflection()) );

$closure = function() {};
Assert::same( $closure, Callback::create($closure)->getNative() );
Assert::same( '{closure}', (string) Callback::create($closure) );
Assert::same( '{closure}', getName(Callback::create($closure)->toReflection()) );
